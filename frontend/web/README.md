...
| `/forgot-password` | 없음 | 가입 이메일로 비밀번호 재설정 링크 요청. 형식 오류는 제출 전·입력 이탈 시 안내하고, 가입되지 않은 이메일은 회원가입으로 안내 |
...
| `/app/welcome` | 가입·첫 로그인 뒤 자동 | 최초 로그인 환영 화면. 회원 유형(개인·기업, 필수)과 이용 목적(선택)을 묻고 `PUT /api/v1/me/onboarding`에 저장. 아직 답하지 않은 계정은 어떤 `/app` 화면을 열어도 먼저 이 화면으로 옴 |
| `/app/pricing` | 계정 메뉴 | 요금제를 사이드바 안에서. 사이드바 업무 메뉴가 아니라 계정 카드 메뉴(내 프로필 · 요금제 · 로그아웃)에서 엽니다. 무료 검색 버튼은 작업 채팅으로 |
| `/app/partners` | 사이드바(협업 · 파트너 모집) | 파트너 모집 목록. 검색어·찾는 역할(복수)·지역(복수, 전체가 전국까지 뜻함)은 조회 버튼으로 적용하고 정렬·페이지는 바로 적용해 모집 API 조회(내 글만 보기 칩은 바로 적용). 카드는 폭에 따라 3·2·1열, 작성 버튼은 기업 등록 회원만 |
| `/app/partners/new` | 사이드바 | 모집글 작성. `관심 공고함에서 선택` 팝업에서 접수 중인 관심 공고 1개를 고르고 등록하면 상세로 이동. 기업 미등록 회원은 프로필 등록 안내 |
| `/app/partners/mine` | (예전 주소) | `/app/partners?mine=1`로 넘어감. 내 글은 목록의 `내가 쓴 모집글만` 칩으로 보고 카드에서 수정·마감 |
...
| `/app/proposals` | 사이드바(협업 · 제안함, 받은 제안 대기 수 배지) | 제안함. 받은 제안 수락·거절, 보낸 제안 철회, 수락된 제안의 상대 담당자 연락처 |
...
| `/app/saved-programs` | 사이드바 | 관심 공고함. 저장한 공고를 목록(마감 임박순, 기본 `/app/saved-programs`)·달력(`?view=calendar`)으로 확인하고, 진행 관리에서 실제 신청 준비 건을 파이프라인의 `준비 중` 단계로 확인함. 공고 제목을 누르면 상세(위 링크가 `← 관심 공고함으로 돌아가기`)로 감 |
...
| `/app/admin/accounts/detail?accountId=...` | 계정 목록 | 목록 위 오른쪽 드로어로 열림(주소 유지). 머리글 `계정 관리 > 계정 상세`·Esc·바깥 클릭으로 목록에 돌아감. 계정·기업·활동·조치 기록과 정지·정지 해제·강제 로그아웃(사유 필수). 내 계정·다른 관리자 계정은 조치 버튼 대신 까닭을 표시 |
