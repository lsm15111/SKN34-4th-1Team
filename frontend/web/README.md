...
| `/app/partners` | 사이드바(협업 · 파트너 모집 · 모집글 탭) | 파트너 모집 목록. 검색어·찾는 역할(복수)·지역(복수, 전체가 전국까지 뜻함)은 조회 버튼으로 적용하고 정렬·페이지는 바로 적용해 모집 API 조회(내 글만 보기 칩은 내 모집글 탭으로 대체). 카드는 폭에 따라 3·2·1열, 작성 버튼은 기업 등록 회원만 |
| `/app/partners/new` | 사이드바 | 모집글 작성. `관심 공고함에서 선택` 팝업에서 접수 중인 관심 공고 1개를 고르고 등록하면 상세로 이동. 기업 미등록 회원은 프로필 등록 안내 |
| `/app/partners/mine` | 사이드바(협업 · 파트너 모집 · 내 모집글 탭) | 내가 쓴 모집글을 최근 등록순으로 모아 상태·받은 제안 수를 보여 주고, 모집 중인 글은 카드에서 수정·마감. 마감된 글은 뒤로 보내되 남김 |
