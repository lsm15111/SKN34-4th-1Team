...
describe('대화 조건 해석·확인 검색 HTTP E2E', () => {
  ...
  it.each([false, true])('로그아웃 후 다른 계정으로 로그인해도 이전 대화·조건·초안이 남지 않는다 (로그아웃 API 실패: %s)', async (logoutFails) => {
    ...
    act(() => store.dispatch(signedIn({ email: 'other@govbiz.local', role: 'USER', tier: 'MEMBER', emailVerified: true, hasPassword: true, accountType: null, onboardingPurpose: null, onboarded: true, company: null })))
